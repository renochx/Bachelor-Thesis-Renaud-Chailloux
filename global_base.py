# Base model, global level, iterative F-tests
import pandas as pd
import os
...
#Step 9: Fit both models and perform hausman test
...
# Step 11: Iteratively perform F-tests and drop insignificant variables
...
# Step 12: Refit the final model with only significant variables
print("\nRefitting the final model with significant variables:")
model = PanelOLS(y, X, entity_effects=True, time_effects=True, check_rank=True)
results = model.fit(cov_type='clustered', cluster_entity=True)
print("Final model fitted with significant variables.")

#Step 13: Fit both models and perform hausman test to ensure FE are still warranted 
#Fit random effects model
def fit_random_effects_model(X, y):
    model = RandomEffects(y, X)
    results = model.fit(cov_type='clustered', cluster_entity=True)
...
# Step 14: Perform Durbin-Watson test
dw_stat = durbin_watson(results.resids)
print("\nDurbin-Watson Test for Autocorrelation:")
print(f"Durbin-Watson statistic: {dw_stat}")

# Step 15: Perform the Breusch-Pagan test 
...
# Step 16: Perform F-test for fixed effects
...
# Step 17: Save regression results to a CSV file
results_df = results.summary.tables[1].as_html()
results_df = pd.read_html(results_df, header=0, index_col=0)[0]
results_df.to_csv('/Users/renaudchailloux/Desktop/Output Data/Base Model//global_base_regression_results.csv')
print("Regression results saved to 'global_base_extended_regression_results.csv'")

# Step 18: Save fixed effects
def save_fixed_effects(results):
    fixed_effects = results.estimated_effects
    fe_unstacked = fixed_effects.unstack(level=0)
    fe_unstacked.to_csv('/Users/renaudchailloux/Desktop/Output Data/Base Model/global_base_unstacked_fixed_effects.csv')
    print("Unstacked fixed effects saved to 'global_base_unstacked_fixed_effects.csv'")
save_fixed_effects(results)

#Step 19: Plot fixed effects
...
plt.grid(True)
plt.tight_layout()
plt.savefig('/Users/renaudchailloux/Desktop/Output Data/Base Model/global_base_time_fixed_effects_plot.png')
plt.show()
# Save the global time fixed effects to a CSV file
time_fixed_effects.to_csv('/Users/renaudchailloux/Desktop/Output Data/Base Model/global_base_time_fixed_effects.csv')
print("Global time fixed effects saved to 'global_base_time_fixed_effects.csv'")
