# Extended model, global level, iterative F-tests
import pandas as pd
import os
...
#Step 14: Fit both models and perform hausman test to ensure FE are still warranted 
#Fit random effects model
def fit_random_effects_model(X, y):
    model = RandomEffects(y, X)
    results = model.fit(cov_type='clustered', cluster_entity=True)
...
results_df.to_csv('/Users/renaudchailloux/Desktop/Output Data/Extended Model//global_extended_regression_results.csv')
print("Regression results saved to 'global_extended_regression_results.csv'")
...
def save_fixed_effects(results):
    ...
    fe_unstacked.to_csv('/Users/renaudchailloux/Desktop/Output Data/Extended Model/global_extended_unstacked_fixed_effects.csv')
    print("Unstacked fixed effects saved to 'global_extended_unstacked_fixed_effects.csv'")
save_fixed_effects(results)

# Step 20: Visualize how the impact of governance indicators changes with GDP
def visualize_interaction_impact_with_gdp(X, y, fitted_values):
    governance_indicators = ['VA', 'PV', 'GE', 'RQ', 'RL', 'CC']
    gdp = X['GDP']
    fitted_values = np.ravel(fitted_values)  # Ensure fitted_values is 1-dimensional
    for indicator in governance_indicators:
        if indicator in X.columns:
            interaction_term = f"{indicator}_GDP_Interaction"
            if interaction_term in X.columns:
                plt.figure(figsize=(10, 6))
                # Create a scatter plot for the interaction term
                sns.scatterplot(x=gdp, y=X[interaction_term], label=f'{indicator} x GDP Interaction', alpha=0.6)
                # Add a line plot for the fitted values
                sns.lineplot(x=gdp, y=fitted_values, color='red', label='Fitted Values')
                # Add titles and labels
                plt.title(f"Distribution of {indicator}*GDP")
                plt.xlabel("GDP (log-transformed)")
                plt.ylabel(f"Interaction term values")
                plt.legend()
                plt.grid(True)
                plt.tight_layout()
                # Save the plot as a file
                plt.savefig(f'/Users/renaudchailloux/Desktop/Output Data/Extended Model/{indicator}_gdp_impact_plot.png')
                plt.show()
                print(f"Visualization for {indicator} x GDP saved as {indicator}_gdp_impact_plot.png")
# Call the function to visualize the interaction effects
visualize_interaction_impact_with_gdp(X, y, results.fitted_values)

# Step 21: Calculate and visualize marginal effects with confidence intervals
def calculate_marginal_effects_with_ci(X, results):
    governance_indicators = ['VA', 'PV', 'GE', 'RQ', 'RL', 'CC']
    gdp = X['GDP']
    params = results.params  # Coefficients from the fitted model
    std_errors = results.std_errors  # Standard errors of the coefficients
    for indicator in governance_indicators:
        if indicator in X.columns:
            interaction_term = f"{indicator}_GDP_Interaction"
            if interaction_term in X.columns:
                # Extract coefficients and standard errors
                beta_main = params[indicator] if indicator in params else 0
                beta_gdp = params['GDP'] if 'GDP' in params else 0
                beta_interaction = params[interaction_term] if interaction_term in params else 0
                se_main = std_errors[indicator] if indicator in std_errors else 0
                se_interaction = std_errors[interaction_term] if interaction_term in std_errors else 0
                # Calculate marginal effects
                marginal_effects = beta_main + beta_interaction * gdp
                # Calculate confidence intervals
                ci_lower = marginal_effects - 1.96 * np.sqrt(se_main**2 + (gdp**2) * se_interaction**2)
                ci_upper = marginal_effects + 1.96 * np.sqrt(se_main**2 + (gdp**2) * se_interaction**2)
                # Find the point of diminishing returns (where marginal effects stop increasing)
                diminishing_point = gdp[np.argmax(marginal_effects < 0)] if np.any(marginal_effects < 0) else None
                # Plot marginal effects with confidence intervals
                plt.figure(figsize=(10, 6))
                plt.plot(gdp, marginal_effects, label='Marginal Effect', color='blue')
                plt.fill_between(gdp, ci_lower, ci_upper, color='blue', alpha=0.2, label='95% Confidence Interval')
                if diminishing_point is not None:
                    plt.axvline(x=diminishing_point, color='red', linestyle='--', label='Diminishing Returns Point')
                # Add titles, labels, and adjust axis scale
                plt.title(f"Marginal Effects of {indicator} with GDP (Interaction Term: {indicator} x GDP)", fontsize=14)
                plt.xlabel("GDP (log-transformed)", fontsize=12)
                plt.ylabel(f"Marginal Effect of {indicator}", fontsize=12)
                plt.xticks(fontsize=10)  # Adjust x-axis tick size
                plt.yticks(fontsize=14)  # Adjust y-axis tick size
                plt.legend(fontsize=10)
                plt.grid(True, linestyle='--', linewidth=0.5)
                plt.tight_layout()
                # Save the plot as a file
                plt.savefig(f'/Users/renaudchailloux/Desktop/Output Data/Extended Model/{indicator}_marginal_effects_with_ci_plot.png')
                plt.show()
                # Print the point of diminishing returns
                if diminishing_point is not None:
                    print(f"Point of diminishing returns for {indicator}: GDP = {diminishing_point}")
                else:
                    print(f"No diminishing returns detected for {indicator} within the range of GDP.")

# Call the function to calculate and visualize marginal effects with confidence intervals
calculate_marginal_effects_with_ci(X, results)

#Step 22: Plot fixed effects
# Extract fixed effects
fixed_effects = results.estimated_effects
# Check if the fixed effects include a time dimension
if 'Year' not in fixed_effects.index.names:
    raise KeyError("The fixed effects do not include a 'Year' level. Please check the model configuration.")
# Extract time fixed effects and average across all entities
time_fixed_effects = fixed_effects.unstack(level='Year').mean(axis=0)  # Average across all entities
time_fixed_effects = time_fixed_effects.sort_index()  # Ensure chronological order
# Ensure the index is a single-level index (Year)
time_fixed_effects.index = time_fixed_effects.index.get_level_values('Year')
# Calculate the global average shift
global_average_shift = time_fixed_effects.mean()
print(f"\nGlobal average shift in the dependent variable over the sample period (mean of time fixed effects): {global_average_shift:.4f}")
# Plot the global time fixed effects
plt.figure(figsize=(10, 6))
plt.plot(time_fixed_effects.index, time_fixed_effects.values, marker='o', linestyle='-', label='Global Time Fixed Effects')
plt.axhline(global_average_shift, color='red', linestyle='--', label=f'Global Average Shift ({global_average_shift:.2f})')
plt.title('Global Time Fixed Effects: Shift in Renewable Energy Consumption Over Time')
plt.xlabel('Year')
plt.ylabel('Global Time Fixed Effect')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig('/Users/renaudchailloux/Desktop/Output Data/Extended Model/global_extended_time_fixed_effects_plot.png')
plt.show()
# Save the global time fixed effects to a CSV file
time_fixed_effects.to_csv('/Users/renaudchailloux/Desktop/Output Data/Extended Model/global_extended_time_fixed_effects.csv')
print("Global time fixed effects saved to 'global_extended_time_fixed_effects.csv'")
